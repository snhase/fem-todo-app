...
import { manageTodo, deleteTodo } from "../utils/requests.tsx";
...
const TaskList = ({ filterType, toDoList, setToDoList }: Props) => {
  ...
  const visibleToDo = useMemo(
    () => filterTodo(toDoList, filterType),
    [toDoList, filterType]
  );
  ...
  const markToDoComplete = (task: ToDo) => {
    ...
    manageTodo(apiPath, "PUT", JSON.stringify(task), () => {
      setToDoList([...rest.slice(0, index), task, ...rest.slice(index)]);
    });
  };
  ...
  const handleDrop = (event: DragEvent<HTMLDivElement>) => {
    ...
    const newDragPosition = toDoList.findIndex(
      (todo) => todo.id === visibleToDo[newDragIndex].id
    );
    if (dragFrom !== newDragPosition) {
      let updatedList = toDoList;
      const taskDragged = visibleToDo[dragFrom];
      const rest = updatedList.filter(
        (item: ToDo) => item.id !== taskDragged.id
      );
      updatedList = [
            ...
            >
              ...
              >
                {task.completed ? (
                  <div className="h-6 w-6 bg-gradient-to-br from-[hsl(192,100%,67%)] to-[hsl(280,87%,65%)] rounded-full hover:cursor-pointer">
                    <img
                      src={CheckIcon}
                      className="h-25 w-25 p-2 absolute"
                      alt="i"
                    ></img>
                  </div>
                ) : (
                  <div className="p-0.5 hover:cursor-pointer hover:bg-gradient-to-br from-[hsl(192,100%,67%)] to-[hsl(280,87%,65%)] rounded-full">
                    <div className="h-6 w-6 bg-white dark:bg-veryDarkDesaturatedBlue rounded-full border-2 border-slate-200 dark:border-veryDarkGrayishBlue2 hover:border-0"></div>
                  </div>
                )}
              </span>
              <div
                className={`
                        ...
                        ${
                          task.completed
                            ...
                            : "text-veryDarkGrayishBlue dark:text-lightGrayishBlue"
                        }
                        ${
                          dragActive &&
                          (dragFrom === Number(idx) || dragFrom !== Number(idx))
                            ? "cursor-grabbing"
              ...
              >
                ...
                {showDelete === task.id ? (
                  ...
                  >
                    <svg
                      className="absolute inset-y-0 my-auto right-5 fill-[#494C6B] hover:fill-red-600"
